...
class VisionTransformerUSFM(nn.Module):
    ...
    def get_intermediate_layers(self, x, n=1, reshape=False, return_class_token=False, norm=True):
        # Prepare tokens like in forward_features
        x = self.patch_embed(x)
        B = x.shape[0]
        ...
        blocks_to_take = list(range(total_blocks - n, total_blocks)) if isinstance(n, int) else list(n)
        # Iterate blocks and collect intermediate outputs
        for i, blk in enumerate(self.blocks):
            # Assume no relative bias adjustment for simplicity
            x = blk(x)
            if i in blocks_to_take:
                out = x
                if norm and hasattr(self, "norm") and self.norm is not None:
                    out = self.norm(out)
                if reshape:
                    H, W = self.patch_embed.patch_shape
                    out = out[:, 1:, :].reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()
