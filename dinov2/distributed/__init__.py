...
def _collect_env_vars(environment="torchrun") -> Dict[str, str]:
    if environment == "MPI":
        return {env_var: os.environ[env_var] for env_var in _MPI_ENV_VARS if env_var in os.environ}
    else:
...
class _TorchDistributedEnvironment:
    def __init__(self):
        ...
        if not os.environ["MASTER_ADDR"] and not os.environ["MASTER_PORT"]:
            if torch.cuda.device_count() > 0:
                return self._set_from_local()
            else:
                raise RuntimeError("No available GPUs")

        runner = os.environ.get("RUNNER") or "torchrun"
        env_vars = _collect_env_vars(runner)

        if len(env_vars) == len(_TORCH_DISTRIBUTED_ENV_VARS) or len(env_vars) == len(_MPI_ENV_VARS):
            # Environment is fully set
            return self._set_from_preset_env(runner=runner)
        else:
    ...
    def _set_from_preset_env(self, runner: str):
        if runner == "MPI":
            print("Initialization from MPI environment")
            ...
            self.local_world_size = int(os.environ["OMPI_COMM_WORLD_LOCAL_SIZE"])

        elif runner == "torchrun":
            print("Initialization from torchrun environment")
            self.master_addr = os.environ["MASTER_ADDR"]
            self.master_port = os.environ["MASTER_PORT"]
