...
def count_open_files_in_workers():
    # Get the current process
    current_process = psutil.Process()

    # Get all child processes (DataLoader workers)
    child_processes = current_process.children()

    # Count open file descriptors for each child process
    open_files_count = sum(len(proc.open_files()) for proc in child_processes if proc.is_running())
                ...
                if torch.cuda.is_available():
                    logger.info(
                        log_msg.format(
                            ...
                            open_files_count=count_open_files_in_workers(),
                        )
                    )
                else:
                    logger.info(
