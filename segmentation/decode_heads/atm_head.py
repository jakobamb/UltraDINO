...
class TPN_DecoderLayer(TransformerDecoderLayer):
    def __init__(self, **kwargs):
        ...
        self.multihead_attn = Attention(kwargs["d_model"], num_heads=kwargs["nhead"], qkv_bias=True, attn_drop=0.1)
    ...
    def forward(
        ...
        tgt = self.norm1(tgt)
        tgt2, attn2 = self.multihead_attn(tgt.transpose(0, 1), memory.transpose(0, 1), memory.transpose(0, 1))
...
class ATMHead(BaseDecodeHead):
    def __init__(
        ...
        **kwargs,
        ...
        self.class_embed = nn.Linear(dim, self.num_classes)
    ...
    def forward(self, inputs):
        ...
        for idx, (x_, proj_, norm_, decoder_) in enumerate(zip(x, self.input_proj, self.proj_norm, self.decoder)):
            ...
            if self.crop_train and self.training:
                ...
                blank_attn = blank_attn[:, 0].unsqueeze(1).repeat(1, (self.image_size[0] // 16) ** 2, 1)
        ...
        for i_attn, attn in enumerate(attns):
            if i_attn == 0:
                ...
                outputs_seg_masks.append(
                    outputs_seg_masks[i_attn - 1] + F.interpolate(attn, size=size, mode="bilinear", align_corners=False)
                )

        out["pred_masks"] = F.interpolate(
            outputs_seg_masks[-1], size=self.image_size, mode="bilinear", align_corners=False
        )
    ...
    # Overwrite base methods so we can use ATM loss
    def loss_by_feat(self, seg_logits: dict, batch_data_samples: SampleList) -> dict:
        ...
        loss = dict()
        loss = self.loss_decode(seg_logits, seg_label, ignore_index=self.ignore_index)

        loss["acc_seg"] = accuracy(seg_logits["pred"], seg_label, ignore_index=self.ignore_index)
        return loss
