...
class DinusIoUMetric(BaseMetric):
    """DINUS Version of the IoU evaluation metric.
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        for ret_metric, ret_metric_value in ret_metrics.items():
            # Mean including foreground (all indices)
            ret_metrics_summary[ret_metric] = np.round(np.nanmean(ret_metric_value) * 100, 2)
            # Mean excluding foreground (i.e. ignore index 0)
            if ret_metric_value.ndim > 0:
                ret_metrics_summary["f" + ret_metric] = np.round(np.nanmean(ret_metric_value[1:]) * 100, 2)

        metrics = dict()
        for key, val in ret_metrics_summary.items():
            if key in ["aAcc", "aDice", "aIoU"]:
                metrics[key] = val
            else:
                metrics["m" + key] = val
    ...
    def total_area_to_metrics(
        ...
        if isinstance(metrics, str):
            metrics = [metrics]
        allowed_metrics = ["mIoU", "mDice", "mFscore", "cmat"]
        if not set(metrics).issubset(set(allowed_metrics)):
            raise KeyError(f"metrics {metrics} is not supported")

        foreground_area_intersect = total_area_intersect[1:].sum()
        foreground_area_union = total_area_union[1:].sum()
        foreground_area_label = total_area_label[1:].sum()
        foreground_area_pred_label = total_area_pred_label[1:].sum()
        ...
        for metric in metrics:
            if metric == "mIoU":
                ...
                all_iou = foreground_area_intersect / foreground_area_union
                all_acc = foreground_area_intersect / foreground_area_label
                ...
                all_dice = 2 * foreground_area_intersect / (foreground_area_pred_label + foreground_area_pred_label)
                ret_metrics["Dice"] = dice
                ret_metrics["aDice"] = all_dice
                ...
                ret_metrics["TP"] = total_area_intersect
                ret_metrics["FP"] = total_area_pred_label - total_area_intersect
                ret_metrics["FN"] = total_area_label - total_area_intersect
